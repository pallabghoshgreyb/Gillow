...
const TechMap: React.FC<TechMapProps> = ({ nodes, onSelectNode, selectedNodeId }) => {
  ...
  useEffect(() => {
    ...
    const initialTransform = zoomIdentity
      .translate(wrapperWidth / 2 - (MAP_WIDTH / 2) * initialScale, wrapperHeight / 2 - (MAP_HEIGHT / 2) * initialScale)
      .scale(initialScale);
    ...
    updateVisibility(initialScale);

  }, [nodes, selectedNodeId, onSelectNode, scatterPoints]);

  return (
    <div ref={wrapperRef} className="w-full h-full relative bg-white overflow-hidden">
      <div className="absolute top-8 left-8 z-10 space-y-4">
        <div className="bg-white/95 backdrop-blur-xl p-5 rounded-3xl border border-slate-200 shadow-xl w-64">
      ...
      <div className="absolute bottom-8 right-8 flex flex-col gap-3 z-10">
        ...
        <button onClick={() => svgRef.current && zoomRef.current && select(svgRef.current).transition().duration(750).call(zoomRef.current.transform, zoomIdentity.translate(wrapperRef.current!.clientWidth / 2 - (MAP_WIDTH / 2) * 0.4, wrapperRef.current!.clientHeight / 2 - (MAP_HEIGHT / 2) * 0.4).scale(0.4))} className="w-12 h-12 flex items-center justify-center bg-white border border-slate-200 rounded-2xl text-slate-400 hover:text-blue-600 shadow-xl active:scale-95 transition-all">
          <Compass size={24} />
        </button>
      </div>
