...
const TechMap: React.FC<TechMapProps> = ({ nodes, onSelectNode, selectedNodeId, showLegend = true }) => {
  ...
  const getOverviewTransform = (width: number, height: number) => {
    ...
    const minScaleForViewportHeight = height / MAP_HEIGHT;
    const overviewScale = Math.max(minScaleForViewportHeight, 0.28, Math.min(Math.min(usableWidth / boundsWidth, usableHeight / boundsHeight) * 0.94, 1.1));
  ...
  useEffect(() => {
    ...
    nodes.forEach((node, i) => {
        ...
        const angle = isDomain
          ? ((i % 4 === 0 ? 1 : i % 4 === 1 ? -1 : i % 4 === 2 ? 1 : -1) * (Math.PI / 4) + (Math.PI / 8))
          : (Math.PI / 6 + (i % 2) * Math.PI);
    ...
    setTimeout(() => updateVisibility(initialTransform.k), 50);
