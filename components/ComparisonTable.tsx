...
const ComparisonTable: React.FC<ComparisonTableProps> = ({ patentIds }) => {
  ...
  const rows = [
    { label: 'Market Valuation', key: 'askingPrice', format: (val: number) => formatCompactCurrency(val), icon: <TrendingUp size={16}/> },
      ...
      <table className="w-full text-left border-collapse">
        <thead>
          <tr className="bg-slate-50">
            <th className="w-1/4 border-b border-slate-200 p-6 text-xs font-medium uppercase tracking-[0.16em] text-slate-400">Specifications</th>
            {patents.map(p => (
              <th key={p.publicationNumber} className="p-6 border-b border-slate-200 min-w-[200px]">
                <div className="flex flex-col gap-2">
                  <div className="text-xs font-medium text-blue-600">{p.publicationNumber}</div>
                  <div className="h-10 line-clamp-2 text-sm font-semibold leading-tight text-slate-900">{p.title}</div>
                </div>
              </th>
            ))}
          </tr>
        ...
        <tbody>
          {rows.map((row, idx) => (
            <tr key={row.label} className={idx % 2 === 0 ? 'bg-white' : 'bg-slate-50/30'}>
              <td className="flex items-center gap-3 border-b border-slate-100 p-6 text-sm font-medium text-slate-500">
                <span className="text-slate-300">{row.icon}</span>
                {row.label}
              </td>
              {patents.map(p => (
                <td key={p.publicationNumber} className="p-6 border-b border-slate-100">
                  <span className={`text-sm font-semibold ${row.key === 'askingPrice' ? 'text-blue-600' : 'text-slate-900'}`}>
          ...
          <tr>
            <td className="p-6 text-sm font-medium text-slate-500">Tech Abstract</td>
            {patents.map(p => (
