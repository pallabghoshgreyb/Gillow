...
const operationalStatusText = (patent: Patent) =>
  patent.simpleLegalStatus || patent.legalStatus || 'Unknown';

const isOperationallyInactive = (status: string) =>
  /\b(inactive|dead|expired|lapsed)\b/i.test(status);

const isOperationallyActive = (status: string) =>
...
const PatentCard: React.FC<PatentCardProps> = ({ patent, isFavorite, onToggleFavorite, onClick, href, layout = 'grid' }) => {
  ...
  const statusTone = isOperationallyInactive(operationalStatus)
    ...
    : isOperationallyActive(operationalStatus)
      ? 'text-emerald-600'
      : 'text-amber-600';
  const statusDot = isOperationallyInactive(operationalStatus)
    ? 'bg-red-500'
    : isOperationallyActive(operationalStatus)
