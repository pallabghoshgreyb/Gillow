...
const PatentCard: React.FC<PatentCardProps> = ({ patent, isFavorite, onToggleFavorite, onClick, layout = 'grid' }) => {
  const isList = layout === 'list';

  return (
    <div 
      className={`group bg-white rounded-xl border border-slate-200 overflow-hidden shadow-sm hover:shadow-2xl hover:-translate-y-1.5 transition-all duration-300 cursor-pointer flex ${isList ? 'flex-row h-72' : 'flex-col h-full'}`}
      onClick={() => onClick?.(patent.publicationNumber)}
    >
      {/* Technical Header */}
      <div className={`relative overflow-hidden flex-shrink-0 flex flex-col items-center justify-center bg-slate-50 ${isList ? 'w-80 border-r border-slate-100' : 'h-48 border-b border-slate-100'}`}>
        ...
        <button 
          onClick={(e) => { e.stopPropagation(); onToggleFavorite?.(patent.publicationNumber); }}
