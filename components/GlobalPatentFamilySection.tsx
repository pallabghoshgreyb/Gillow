...
const inferStatus = (publicationNumber: string, patent: Patent): FamilyStatus => {
  ...
  if (normalized === currentPublication) {
    if (isOperationallyInactive(operationalStatusText(patent))) return 'Expired';
    if (hasGrantedPatentType(patent.patentType) || hasGrantedKindCode(normalized)) return 'Granted';
    return 'Pending';
  }

  if (hasGrantedKindCode(normalized) || /(?:C\d?|T\d?)$/.test(normalized)) return 'Granted';
  if (/(?:A\d?)$/.test(normalized)) return 'Pending';
