...
const resolvePublicationStatus = (
  ...
  const isCurrentPublication = normalized === normalizeUpper(patent.publicationNumber);

  if (isCurrentPublication && isOperationallyInactive(operationalStatusText(patent))) {
    return { status: 'expired', label: operationalStatusLabel(patent) };
  }

  if ((isCurrentPublication && hasGrantedPatentType(patent.patentType)) || hasGrantedKindCode(normalized) || /(?:C\d?|T\d?)$/.test(normalized)) {
    return { status: 'granted', label: 'Granted' };
  }
...
const buildGroups = (patent: Patent): CountryGroup[] => {
  ...
  members.forEach((publicationNumber) => {
    ...
    const resolvedStatus = resolvePublicationStatus(publicationNumber, patent);
