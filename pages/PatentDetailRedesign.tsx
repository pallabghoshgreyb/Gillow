...
const operationalStatusText = (patent: Pick<Patent, 'legalStatus' | 'simpleLegalStatus'>) =>
  `${patent.legalStatus || ''} ${patent.simpleLegalStatus || ''}`.trim();

const isOperationallyInactive = (status: string) =>
  /\b(inactive|dead|expired|lapsed)\b/i.test(status);

const operationalStatusLabel = (patent: Patent) =>
  hasText(patent.simpleLegalStatus)
    ? patent.simpleLegalStatus
    : hasText(patent.legalStatus)
      ? patent.legalStatus
      : 'Inactive';

const hasGrantedKindCode = (publicationNumber: string) =>
  /(?:B[12]?|S1?)$/.test(normalizeUpper(publicationNumber));

const hasGrantedPatentType = (patentType?: string | null) =>
  /\b(granted|issued)\b/i.test(patentType || '');

const statusOf = (patent: Patent) => {
  if (isOperationallyInactive(operationalStatusText(patent))) {
    return { label: operationalStatusLabel(patent), tone: 'expired' as Tone };
  }

  if (hasGrantedPatentType(patent.patentType) || hasGrantedKindCode(patent.publicationNumber)) {
    return { label: 'Granted', tone: 'granted' as Tone };
  }
...
const continuityStatusOf = (
  ...
  if (matchedPatent && isOperationallyInactive(operationalStatusText(matchedPatent))) {
    return { label: operationalStatusLabel(matchedPatent), tone: 'expired' };
  }

  if (matchedPatent && hasGrantedPatentType(matchedPatent.patentType)) return { label: 'Granted', tone: 'granted' };
  if (hasGrantedKindCode(normalized) || /(?:C\d?|T\d?)$/.test(normalized)) return { label: 'Granted', tone: 'granted' };
    ...
    },
    {
      ...
      detail:
        status.tone === 'granted'
          ...
          : status.tone === 'expired'
            ? 'The asset is marked inactive or dead in the operational status fields.'
...
const buildHistory = (patent: Patent, status: { label: string; tone: Tone }): History[] => [
  ...
  {
    id: 'grant',
    title: status.tone === 'granted' ? 'Grant / issuance' : status.tone === 'expired' ? 'Operational status' : 'Grant posture',
    ...
    detail: status.tone === 'granted' ? 'The file reflects an issued posture suitable for licensing and enforcement review.' : status.tone === 'expired' ? 'The record is marked inactive or dead in the operational status fields.' : 'Final grant outcome should be confirmed with the latest official prosecution record.',
