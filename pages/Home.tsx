...
import { 
  Search, TrendingUp, Tag, ArrowRight, Heart, 
    ...
    <div className="flex flex-col min-h-screen bg-white font-sans">
      ...
      <section className="relative min-h-screen flex flex-col items-center justify-start pt-32 pb-20 overflow-hidden bg-slate-50 border-b border-slate-200">
          ...
          <div className="z-10 w-full max-w-[1400px] px-6 text-center mb-16 relative">
               ...
               <form onSubmit={handleSearch} className="relative group max-w-2xl mx-auto shadow-2xl rounded-full z-30">
                   <input 
                       type="text"
                       value={search}
                       onChange={(e) => setSearch(e.target.value)}
                       placeholder="Search patents, companies, or tech clusters..."
                       className="w-full h-20 pl-10 pr-20 rounded-full border-2 border-slate-100 text-lg font-medium focus:outline-none focus:border-[#006AFF] transition-all bg-white shadow-inner"
                   />
                   <button 
                       type="submit"
                       className="absolute right-3 top-3 w-14 h-14 bg-[#006AFF] text-white rounded-full flex items-center justify-center hover:bg-blue-700 transition-all shadow-lg active:scale-95"
                   >
