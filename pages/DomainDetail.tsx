...
import {
  ...
  domainPublicationYearForPatent,
...
const AssigneeCompetitiveLandscape = ({
  domain,
  onSelectAssignee,
}: {
  ...
  if (domain.patents.length === 0) {
    return (
      <section className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
        <EmptyState title="No assignee data" text="Assignee information is not available for this domain yet." />
      </section>
    );
  }

  return (
    <section className="h-[600px] overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm md:h-[700px]">
      <BubbleChart
        patents={domain.patents}
...
const DomainDetail: React.FC = () => {
  ...
  const filterOptions = useMemo(
    () =>
      domain
        ...
        : { technologies: [], companies: [], years: [], statuses: [] },
    [domain],
  );
  ...
  const filteredPatents = useMemo(() => {
    ...
    return domain.patents
      .filter((patent) => {
        ...
        if (selectedYear !== 'all' && String(domainPublicationYearForPatent(patent) || '') !== selectedYear) return false;
  ...
  }, [domain, query, selectedCompany, selectedStatus, selectedTechnology, selectedYear, sortBy]);
  ...
  const trendSlice = useMemo(() => (domain ? domain.trends : []), [domain]);
  const maxTrendCount = Math.max(...trendSlice.map((trend) => trend.count), 1);
  const trendChartHeight = Math.min(280, Math.max(190, 150 + trendSlice.length * 8));
  const trendMaxBarHeight = Math.max(90, trendChartHeight - 34);
  const trendChartMinWidth = Math.max(720, trendSlice.length * 88);
  ...
  return (
    <div className="min-h-screen bg-white text-slate-900">
      ...
      <main className="mx-auto max-w-[1320px] space-y-12 px-4 py-12 sm:px-6 lg:px-8">
        ...
        <AssigneeCompetitiveLandscape
          domain={domain}
          onSelectAssignee={(assignee) => {
            setSelectedCompany(assignee);
            document.getElementById('domain-patents')?.scrollIntoView({ behavior: 'smooth', block: 'start' });
          }}
        />
        ...
        <section className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
          ...
          {trendSlice.length > 0 ? (
            <div className="overflow-x-auto pb-2">
              <div className="flex items-end gap-3" style={{ minWidth: `${trendChartMinWidth}px` }}>
                {trendSlice.map((trend) => {
                  const height = Math.max(14, Math.round((trend.count / maxTrendCount) * trendMaxBarHeight));
                  const labelOffset = height >= trendMaxBarHeight - 8 ? 4 : 8;
                  return (
                    <div key={trend.year} className="flex flex-1 flex-col items-center gap-3">
                      <div
                        className="relative flex w-full items-end rounded-xl bg-slate-50 px-2"
                        style={{ height: `${trendChartHeight}px` }}
                      >
                        <p
                          className="absolute left-1/2 -translate-x-1/2 text-sm font-semibold tabular-nums text-slate-900"
                          style={{ bottom: `${height + labelOffset}px` }}
                        >
                          {trend.count}
                        </p>
                        <div
        ...
        <section className="space-y-6" id="domain-patents" ref={patentsSectionRef} style={{ scrollMarginTop: '96px' }}>
          ...
          <div className="rounded-2xl border border-slate-200 bg-slate-50 p-4 shadow-sm">
            <div className="grid gap-4 lg:grid-cols-[minmax(240px,1.2fr)_repeat(4,minmax(150px,1fr))_150px]">
              <label className="flex min-w-0 flex-col gap-1">
                ...
                <span className="relative">
                  ...
                  <input
                    ...
                    placeholder="Title, patent number, assignee..."
              ...
              <FilterSelect label="Publication Year" value={selectedYear} options={filterOptions.years} onChange={setSelectedYear} />
